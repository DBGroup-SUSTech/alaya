...
struct TimerHelper {
  static uint64_t average(uint64_t low, uint64_t high) { return low + (high - low) / 2; }
...
inline std::tuple<uint64_t, std::chrono::steady_clock::time_point> TimerHelper::base_point =
    TimerHelper::get_tsc_ns_pair();
...
class Timer : protected TimerHelper {
 public:
  Timer() { memset(&cycles_arr_, 0, sizeof(cycles_arr_)); }

  struct end_tag_t {};

  template <typename... Ns>
    requires((std::is_convertible_v<Ns, size_t> || std::is_same_v<Ns, end_tag_t>) && ...) &&
  ...
  std::chrono::duration<Rep, Period> get(size_t i) {
    ...
    std::chrono::nanoseconds result{
        static_cast<std::chrono::nanoseconds::rep>(cycles_arr_[i] * scale)};
    return std::chrono::duration_cast<std::chrono::duration<Rep, Period>>(result);
  }

  static size_t size() { return N; }

  uint64_t get_cycle(size_t i) { return cycles_arr_[i]; }
