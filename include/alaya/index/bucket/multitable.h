...
int Multitable<T>::GetCellGlobalIndex(const vector<int>& indices) const {
  ...
  int global_index = 0;
  int subtable_capacity = table.size();  // 100
  for (int dimension_index = 0; dimension_index < dimensions.size();
       ++dimension_index)  // dimensions.size=2
  {
...
void Multitable<T>::Resize(const vector<int>& new_dimensions, T value) {
  int table_size = 1;
  // new_dimensions 是一个数组，存储的是每个维度的中心点个数
